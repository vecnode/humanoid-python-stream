...
class CLoSD(humanoid_im.HumanoidIm):
    ...
    def get_mdm_next_planning_horizon(self):
        ...
        def _viewer_heartbeat():
            if self.viewer is None:
                return
            # Keep OS window events flowing while diffusion runs.
        ...
        sample = self.sample_fn(
            ...
            const_noise=False,
            cond_fn=cond_fn,
            heartbeat_fn=_viewer_heartbeat,
        )  # [bs, 263, 1, 60]
        ...
        dip_cfg = self.cfg['env']['dip']
        if dip_cfg.get('debug_hml', False) and text_prompts_changed:
            print(f'in get_mdm_next_planning_horizon: prompts={self.hml_prompts[:2]}')
            if dip_cfg.get('save_debug_mp4', False):
    ...
    def save_episodes(self):
        ...
        if self.cfg['env']['dip'].get('debug_hml', False) and self.cfg['env']['dip'].get('save_debug_mp4', False) and hml_motions.shape[0] >= 12:  
            n_vis = 3
            n_files = 4
    ...
    def visualize(self, pose, out_path, is_prefix_comp=False, rep_type='hml', model_kwargs=None, prompts=None):
        ...
        if rep_type == 'hml':
            # keep batch dimension for bs=1 debug runs
            if pose.dim() == 4:
                # [bs, 263, 1, T] -> [bs, T, 263]
                pose_hml = pose.squeeze(2).permute(0, 2, 1)
            elif pose.dim() == 3:
                # already [bs, T, 263]
                pose_hml = pose
            else:
                raise ValueError(f'Unexpected pose shape for hml visualization: {tuple(pose.shape)}')
            pred_xyz = recover_from_ric((pose_hml * self.std + self.mean).float(), 22)
        elif rep_type == 'xyz':
            pred_xyz = self.rep.pose_to_hml_xyz(pose)
        else:
            raise ValueError()
        # Normalize to [bs, seqlen, 22, 3] for consistent downstream plotting.
        if pred_xyz.dim() == 5:
            pred_xyz = pred_xyz.squeeze(2)
        elif pred_xyz.dim() == 3:
            pred_xyz = pred_xyz.unsqueeze(0)
        xyz = pred_xyz.detach().cpu().numpy()
